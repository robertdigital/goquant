...
from gateways.alpaca import AlpacaGateway, AlpacaOrder
...
def algo1(alpaca, position_size=100, max_positions=5):
    ...
    '''
    price_df = alpaca.get_prices(Universe)
    ...
    for symbol in to_sell:
        ...
        orders.append(AlpacaOrder(symbol=symbol, qty=shares, side='sell'))
        logging.info(f'order(sell): {symbol} for {shares}')
    ...
    for symbol in to_buy:
        ...
        if shares == 0.0:
            continue
        orders.append(AlpacaOrder(symbol=symbol, qty=shares, side='buy'))
        logging.info(f'order(buy): {symbol} for {shares}')
        max_to_buy -= 1
    return orders
