...
class BinanceSocketManager(threading.Thread):
    ...
    def start_miniticker_socket(self, callback, update_time=1000):
        ...
        return self._start_socket(
            '!miniTicker@arr@{}ms'.format(update_time), callback)
    ...
    def start_margin_socket(self, callback):
        ...
        # and start the socket with this specific key
        return self._start_account_socket(
            'margin', margin_listen_key, callback)
    ...
    def stop_socket(self, conn_key):
        ...
        if len(
                conn_key) >= 60 and conn_key[:60] == self._listen_keys['margin']:
