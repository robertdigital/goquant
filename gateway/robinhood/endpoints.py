...
def ach(option):
    ...
    Combination of 3 ACH endpoints. Options include:
        ...
        * transfers
    '''
    return api_url + "/ach/iav/auth/" if option == "iav" else api_url + \
        "/ach/{_option}/".format(_option=option)
...
def chain(instrumentid):
    return api_url + \
        "/options/chains/?equity_instrument_ids={_instrumentid}".format(
...
def market_data(optionid):
    return api_url + \
        "/marketdata/options/{_optionid}/".format(_optionid=optionid)
