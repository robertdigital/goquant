...
class Robinhood:
    ...
    def get_historical_quotes(self, stock, interval,
                              span, bounds=Bounds.REGULAR):
    ...
    def get_historical_quotes_df(
            self, stocks, interval, span, bounds=Bounds.REGULAR):
        ret = pd.DataFrame()
        for s in stocks:
            cur_p = self.get_historical_quotes(stock=s,
                                               interval=interval,
                                               span=span,
                                               bounds=bounds)
            df1 = pd.DataFrame.from_records(cur_p['historicals'])
    ...
    def get_news(self, stock):
        ...
        return self.session.get(endpoints.news(
            stock.upper()), timeout=15).json()
    ...
    def get_tickers_by_tag(self, tag=None):
        ...
        return [self.get_url(instrument)["symbol"]
    ...
    def get_options(self, stock, expiration_dates, option_type):
        ...
        chain_id = self.get_url(endpoints.chain(instrument_id))[
            "results"][0]["id"]
        return [contract for contract in self.get_url(endpoints.options(
            chain_id, _expiration_dates_string, option_type))["results"]]
    ...
    def submit_order(self,
        ...
        # Price is required, so we use the current bid price if it is not
        # specified
        ...
        if(instrument_URL is None):
            if(symbol is None):
                raise(
                    ValueError('Neither instrument_URL nor symbol were passed to submit_order'))
        ...
        if(order_type == 'limit'):
            if(price is None):
                raise(
                    ValueError('Limit order has no price in call to submit_order'))
            if(price <= 0):
                raise(
                    ValueError('Price must be positive number in call to submit_order'))

        if(trigger == 'stop'):
            if(stop_price is None):
                raise(
                    ValueError('Stop order has no stop_price in call to submit_order'))
            if(stop_price <= 0):
                raise(
                    ValueError('Stop_price must be positive number in call to submit_order'))

        if(stop_price is not None):
            if(trigger != 'stop'):
                raise(
                    ValueError('Stop price set for non-stop order in call to submit_order'))
        ...
        if(price is not None):
            if(order_type.lower() == 'market'):
                raise(
                    ValueError('Market order has price limit in call to submit_order'))
            price = float(price)
        else:
        ...
        if(quantity <= 0):
            raise(
                ValueError('Quantity must be positive number in call to submit_order'))
